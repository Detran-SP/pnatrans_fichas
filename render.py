...
from app.pdf_generator import generate_csvs_zip, generate_documents
...
def main():
    ...
    pdf_files = generate_documents(df, output_dir, fmt="pdf", on_progress=on_progress)
    print(f"\nConcluído! {len(pdf_files)} relatórios gerados na pasta 'output/'.")

    csv_zip_path = output_dir / "produtos_pnatrans.zip"
    generate_csvs_zip(df, csv_zip_path)
    print(f"CSVs exportados em '{csv_zip_path}'.")
